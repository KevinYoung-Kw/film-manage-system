...
# 重置数据库
echo "步骤 1/3: 执行 DATABASE_RESET.sql"
PGPASSWORD=$DB_PASS psql -h $DB_URL -p $DB_PORT -d $DB_NAME -U $DB_USER -f DATABASE_RESET.sql

# 应用认证和安全设置
echo "步骤 2/3: 执行 auth_and_security.sql"
PGPASSWORD=$DB_PASS psql -h $DB_URL -p $DB_PORT -d $DB_NAME -U $DB_USER -f auth_and_security.sql

# 应用最新的迁移文件
echo "步骤 3/3: 执行 20250411_fix_ticket_operations.sql"
PGPASSWORD=$DB_PASS psql -h $DB_URL -p $DB_PORT -d $DB_NAME -U $DB_USER -f 20250411_fix_ticket_operations.sql
